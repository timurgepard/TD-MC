...
class DDPG():
    ...
    def TD(self):
        self.tr += 1
        St, At, Rt, St_, gamma, d = self.replay.sample()
        self.update_target()
        A_ = self.ANN_t(St_)
    ...
    def train(self):
        ...
        for episode in range(self.n_episodes):
            ...
            for t in range(self.max_steps):
                ...
                if len(self.replay.buffer)>=1 and t>=8:
                    ...
                    for ti in range(-1, -8, -1):
                        ...
                        self.replay.buffer[ti][5] = self.gamma**(abs(ti)) #1, 2, 3
                        self.replay.buffer[ti][6] = done
            ...
            print('%d: %f, %f, | %f | replay size %d' % (episode, score, avg_score, self.eps, len(self.replay.pool)))
...
#env = gym.make('HumanoidBulletEnv').env
